...
func (ticker *fakeRuntimeTicker) C() <-chan time.Time { return ticker.ticks }
...
func (cleaner *controlledRequestLogCleaner) Sweep(ctx context.Context, now time.Time) {
	...
	for {
		maximum := cleaner.maxActive.Load()
		if active <= maximum || cleaner.maxActive.CompareAndSwap(maximum, active) {
			break
		}
	}
	cleaner.calls <- now
...
type controlledStageCleaner struct{ calls chan time.Time }
...
func TestRuntimeSweepsRequestLogsImmediatelyAndHourlyWithoutOverlap(t *testing.T) {
	...
	if got := awaitValue(t, cleaner.calls); !got.Equal(base.Add(time.Hour)) {
		t.Fatalf("hourly Sweep time = %v, want %v", got, base.Add(time.Hour))
	}
	cleaner.release <- struct{}{}
	awaitSignal(t, cleaner.returned)
...
func TestRuntimeSweepsCredentialStagesWithoutRequestLogCleaner(t *testing.T) {
	...
	cancel, done := startRuntime(t, runtime)
	if interval := awaitValue(t, created); interval != retentionInterval {
...
func TestRuntimeDoesNotStartBackgroundValidation(t *testing.T) {
	t.Parallel()
	...
	runtime.Run(ctx)
	if called.Load() {
		t.Fatal("runtime started a background ticker without local maintenance work")
	}
}
...
func TestCooldownProblemDoesNotAffectCandidateCollection(t *testing.T) {
	...
	if err := registry.ReplaceCredentials([]state.CredentialEntry{{
		...
		Fingerprint: "test-1", AuthState: state.CredentialAuthStateReady, EncryptedValue: "cipher-one",
	}}); err != nil {
