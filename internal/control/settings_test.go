...
func TestUpdateSettingsRejectsBufferedStreamKey(t *testing.T) {
	t.Parallel()
	fixture := newServiceFixture(t)
	// U003: buffered_stream is retired; UpdateSettings must reject it as unknown.
	_, err := fixture.service.UpdateSettings(t.Context(), SettingsUpdateRequest{
		Settings: map[string]json.RawMessage{"buffered_stream": json.RawMessage("true")},
	})
	if !errors.Is(err, app_errors.ErrValidation) {
		t.Fatalf("UpdateSettings(buffered_stream) error = %v, want ErrValidation", err)
	}
}
...
func TestUpdateSettingsResolvesRetryAndBlacklistPolicies(t *testing.T) {
	...
	assertSettingsPolicyJSON(t, defaults.Values, 5, 3)
