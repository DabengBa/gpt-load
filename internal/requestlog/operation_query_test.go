...
func TestServiceListFiltersLogicalOperationBeforePagination(t *testing.T) {
	...
	completed := time.Now()
	for index, operation := range []execution.Operation{
	...
	query := ListQuery{Operation: execution.OperationProbe, Limit: 1}
	page, err := service.List(context.Background(), query)
	if err != nil || len(page.Items) != 1 ||
		page.Items[0].RequestID != "00000000-0000-4000-8000-000000000703" ||
		page.NextCursor == nil {
	...
	page, err = service.List(context.Background(), query)
	if err != nil || len(page.Items) != 1 ||
		page.Items[0].RequestID != "00000000-0000-4000-8000-000000000701" ||
		page.NextCursor != nil {
