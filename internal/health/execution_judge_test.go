...
func TestJudgeExecutionUsesNeutralEvidenceAndReplayBoundary(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "request-scoped 429 switches candidate without credential penalty",
			attempt: ExecutionAttempt{
				DispatchState: execution.DispatchMaybeSent,
				StatusCode:    http.StatusTooManyRequests,
				Now:           now,
		...
		{
			name: "generic not found switches candidate",
			attempt: ExecutionAttempt{
				DispatchState: execution.DispatchMaybeSent,
				StatusCode:    http.StatusNotFound,
		...
		{
			name: "structured unsupported model 400 switches candidate",
			attempt: ExecutionAttempt{
				DispatchState: execution.DispatchMaybeSent,
				StatusCode:    http.StatusBadRequest,
				Evidence: &execution.ErrorEvidence{
