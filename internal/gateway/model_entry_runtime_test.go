...
func TestHandlerModelScopeCooldownWritesRouteEntryOnly(t *testing.T) {
	...
	handler.applyGroupDecisionEffectForEntry(
		state.GroupView{ID: 1, BlacklistThreshold: 3}, 1, 0, "gpt-4o",
		modelScopeDecision(health.EffectCooldownCredential, until),
		http.StatusNotFound, now,
	)

	view, ok := registry.EntryRuntime(state.RouteEntryKey{GroupID: 1, UpstreamModelID: "gpt-4o"}, now)
	...
	if _, exists := registry.EntryRuntime(state.RouteEntryKey{GroupID: 1, UpstreamModelID: "sibling-model"}, now); exists {
...
func TestHandlerModelScopeFailuresBlacklistRouteEntryOnly(t *testing.T) {
	now := time.Date(2026, time.September, 5, 12, 0, 0, 0, time.UTC)
	handler, registry := newEntryRuntimeTestHandler(t)
	group := state.GroupView{ID: 1, BlacklistThreshold: 2}
...
func TestHandlerCredentialScopeEffectStillWritesCredential(t *testing.T) {
	...
	if _, exists := registry.EntryRuntime(state.RouteEntryKey{GroupID: 1, UpstreamModelID: "gpt-4o"}, now); exists {
		t.Fatal("credential-scope effect created route entry state")
	}
}

func TestHandlerModelScopeWithoutUpstreamModelFallsBackToCredential(t *testing.T) {
	now := time.Date(2026, time.September, 5, 12, 0, 0, 0, time.UTC)
	handler, registry := newEntryRuntimeTestHandler(t)
	until := now.Add(time.Hour)
	...
	if _, exists := registry.EntryRuntime(state.RouteEntryKey{GroupID: 1, UpstreamModelID: ""}, now); exists {
		t.Fatal("empty upstream model created route entry state")
	}
}
