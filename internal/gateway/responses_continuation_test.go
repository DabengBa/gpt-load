...
func TestResponsesContinuationUsesCurrentCandidateIntersection(t *testing.T) {
	...
	if !registry.SetBlacklisted(1) {
		t.Fatal("blacklisting the bound credential failed")
	}
...
func TestResponsesContinuationKeepsBindingAcrossEntryCooldownAndConfigPublication(t *testing.T) {
	forwarder := &scriptedForwarder{results: []UpstreamResult{storedResponse("first"), storedResponse("second")}}
	handler, engine, sink := newContinuationFixture(t, forwarder)
	serveContinuation(t, engine, "gl-client", `{"model":"gpt-4o","input":"initial"}`, http.StatusOK)
	handler.manager.Current().Groups[1].Models[0].EntryID = "e000000000001"
	registry := handler.registry.(*state.CredentialRegistry)
	if ok, _ := registry.SetEntryCooldownForEntry(1, "e000000000001", time.Now().Add(time.Hour)); !ok {
		t.Fatal("set entry cooldown failed")
	}
	...
	if ok := registry.RecoverEntryForEntry(1, "e000000000001"); !ok {
		t.Fatal("recovering the entry failed")
	}
...
func TestResponsesContinuationRejectsChangedCredentialIdentity(t *testing.T) {
	...
	if err := registry.ApplyCredentialImport(1, []state.CredentialEntry{{
		ID: 1, GroupID: 1, Version: ref.Version + 1,
...
func TestResponsesContinuationRejectsConflictingResponseBeforeDelivery(t *testing.T) {
	...
	handler, engine, _ := newContinuationFixture(t, forwarder)
	// 第二次请求必须落在另一个凭据上，才能在同一个 access key 内制造归属冲突。
	useAffinityRandomValues(handler, 0, 1, 0)
...
// setContinuationChannel republishes the two-credential fixture onto one channel
// and base URL. Each credential keeps its own group: dev 合同要求每个分组只有一个上游凭据。
func setContinuationChannel(t *testing.T, handler *Handler, channelID channel.ID, baseURL string) {
	...
	for _, id := range []uint{1, 2} {
		groups = append(groups, state.GroupConfig{
			ID: id, Name: string(channelID), ChannelID: channelID, ConnectionType: "api_key",
			Params: params, Models: []state.ModelConfig{{ID: "gpt-4o", EntryID: "e000000000001"}}, Enabled: true,
		})
