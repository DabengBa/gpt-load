...
func TestWebsocketForkAndParentCacheErrorBoundaries(t *testing.T) {
	...
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		for {
			...
			var event []byte
			if code != "" {
				event = []byte(fmt.Sprintf(`{"type":"error","stream_id":%q,"error":{"type":"invalid_parameter","code":%q,"message":"fixture failure"}}`, req.StreamID, code))
			} else {
...
func TestWebsocketRetriesOnlyUnsentUnboundTurns(t *testing.T) {
	for _, dispatch := range []execution.DispatchState{execution.DispatchNotSent, execution.DispatchMaybeSent} {
		t.Run(string(dispatch), func(t *testing.T) {
			...
			// dev 合同要求一分组一上游凭据：两个候选必须落在两个分组里。
			second := input.Groups[0]
			second.ID, second.Name = 2, "ws-two"
			input.Groups = append(input.Groups, second)
			input.Credentials = []state.CredentialConfig{testCredentialConfig(1, 1), testCredentialConfig(2, 2)}
			if _, err := h.manager.Publish(input); err != nil {
				t.Fatal(err)
			}
