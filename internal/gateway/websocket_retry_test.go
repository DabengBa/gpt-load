...
func TestWebsocketBoundRetryableErrorRequestsClientReconnect(t *testing.T) {
	for _, test := range []struct {
		...
		{name: "error event", payload: []byte(`{"type":"error","status":429,"error":{"type":"usage_limit_reached","message":"quota exhausted"}}`),
			evidence: execution.ErrorEvidence{Kind: execution.ErrorKindHTTP, StatusCode: http.StatusTooManyRequests, Code: "upstream_error", OriginHint: execution.ErrorOriginUpstream}, wantEffect: "cooldown_credential"},
		{name: "failed response", payload: []byte(`{"type":"response.failed","response":{"id":"resp_failed","object":"response","status":"failed","model":"upstream","error":{"type":"usage_limit_reached","code":"usage_limit_reached","message":"quota exhausted"}}}`),
			evidence: execution.ErrorEvidence{Kind: execution.ErrorKindHTTP, StatusCode: http.StatusTooManyRequests, Code: "upstream_error", OriginHint: execution.ErrorOriginUpstream}, wantEffect: "cooldown_credential"},
		{name: "explicit request rejection", payload: []byte(`{"type":"error","status":400,"error":{"type":"invalid_parameter","message":"invalid input"}}`),
			evidence: execution.ErrorEvidence{Kind: execution.ErrorKindHTTP, StatusCode: http.StatusBadRequest, Code: "upstream_error", OriginHint: execution.ErrorOriginUpstream}, wantEffect: "none"},
		{name: "unknown replay safety", payload: []byte(`{"type":"error","status":429,"error":{"type":"usage_limit_reached","message":"quota exhausted"}}`),
			evidence: execution.ErrorEvidence{Kind: execution.ErrorKindHTTP, StatusCode: http.StatusTooManyRequests, Code: "upstream_error", OriginHint: execution.ErrorOriginUpstream, ReplaySafety: execution.ReplaySafetyUnknown}, wantEffect: "cooldown_credential"},
	} {
		t.Run(test.name, func(t *testing.T) {
...
func TestWebsocketBoundReconnectPreservesErrorBoundaries(t *testing.T) {
	for _, test := range []struct {
	...
	}{
		{
			...
			errorPayload: []byte(`{"type":"error","error":{"type":"new_provider_error","message":"temporary upstream error"}}`),
			evidence: execution.ErrorEvidence{Kind: execution.ErrorKindProvider,
				Code: "new_provider_error", OriginHint: execution.ErrorOriginUpstream},
		},
		{
...
func TestWebsocketNativeBoundQuotaErrorRequestsClientReconnect(t *testing.T) {
	...
	if turns.Load() != 2 || len(attempts) != 1 || attempts[0].RetryDirective != "next_candidate" ||
		attempts[0].Effect != "cooldown_credential" || attempts[0].WillRetry || attempts[0].Committed {
