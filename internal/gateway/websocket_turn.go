...
func (s *websocketConnection) executeTurn(turn websocketTurn) {
	...
	query := scheduler.Query{ClientProtocol: protocol.OpenAIResponses, Operation: execution.OperationResponsesCreate, RouteRequirement: execution.RouteRequirementNative, ResponsesStorePreference: original.metadata.ResponsesStorePreference, ExternalModel: original.metadata.Model, AccessKey: key, AllowedCredentialIDs: make(map[uint]struct{})}
	...
	for sequence := 1; sequence <= limit; sequence++ {
		...
		proxy, fingerprint, err := resolveAttemptProxy(h.encryption, selection.Group.Proxy)
		if err != nil {
			reject(reasonConfigurationChanged)
			return
		}
		...
		input := ForwardInput{
			...
			attemptIdentity: attemptIdentity{
				RequestID:       id,
				AttemptID:       id + ":" + strconv.Itoa(sequence),
				AttemptSequence: uint32(sequence),
			},
			preparedRoute: preparedRoute{
				...
				ResponsesStorePreference: original.metadata.ResponsesStorePreference,
				RouteMode:                execution.RouteNative,
			},
			attemptEffects: attemptEffects{
				Dialect:        dialect.NewOpenAIResponses(),
				ClientProtocol: protocol.OpenAIResponses,
				ObserveUsage:   effective.metadata.ObserveUsage,
			},
		}
...
func (s *websocketConnection) runWebsocketAttempt(ctx context.Context, cancel context.CancelFunc, binding *websocketBinding, lane string, selection scheduler.Selection, ref state.CredentialRef, input ForwardInput, body []byte, recorder *requestRecorder, unlock func(), firstByteDeadline time.Time, bufferFirstError bool) UpstreamResult {
