...
func (r *CredentialRegistry) ReleaseExpiredBlacklists(
	now time.Time,
	onCredentialReleased ...func(uint),
) (credentials int, entries int) {
	r.mu.Lock()
	releasedCredentialIDs := make([]uint, 0)
	for _, bucket := range r.buckets {
		for _, entry := range bucket {
			...
			entry.FailureGeneration++
			credentials++
			releasedCredentialIDs = append(releasedCredentialIDs, entry.ID)
		}
	}
	for _, state := range r.entryRuntime {
	...
	if len(onCredentialReleased) > 0 && onCredentialReleased[0] != nil {
		for _, credentialID := range releasedCredentialIDs {
			onCredentialReleased[0](credentialID)
		}
	}
