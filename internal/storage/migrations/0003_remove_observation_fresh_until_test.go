...
func TestRemoveObservationFreshUntilMigrationDropsColumnAndPreservesRows(t *testing.T) {
	...
	if err := db.Omit("ProxyConfig", "PriceMultiplierMicros", "ProviderURL").Create(&group).Error; err != nil {
		t.Fatalf("create group: %v", err)
	}
	credential := models.Credential{
		GroupID: group.ID, Data: "cipher", Fingerprint: "fingerprint",
		IdentityFingerprint: "identity", AuthState: models.CredentialAuthStateReady,
	}
