...
func TestDebugCaptureMigrationCreatesQueryableBlobSchema(t *testing.T) {
	...
	if err := migrations.Up0012(db); err != nil {
		t.Fatalf("Up0012() error = %v", err)
	}
	if err := migrations.Validate0012(db); err != nil {
		t.Fatalf("Validate0012() error = %v", err)
	}
	...
	if err := migrations.Up0012(db); err != nil {
		t.Fatalf("repeated Up0012() error = %v", err)
	}
}
...
func TestDebugCaptureMigrationValidationRejectsMissingCompositeForeignKey(t *testing.T) {
	...
	if err := migrations.Validate0012(db); err == nil {
		t.Fatal("Validate0012() error = nil, want missing composite foreign key error")
	} else if !strings.Contains(err.Error(), "composite capture/attempt foreign key") {
		t.Fatalf("Validate0012() error = %v, want composite foreign key error", err)
	}
}
