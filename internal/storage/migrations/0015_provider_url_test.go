...
func TestProviderURLMigrationAddsNullableTextColumnAndPreservesRows(t *testing.T) {
	...
	if err := migrations.Up0015(db); err != nil {
		t.Fatalf("Up0015() error = %v", err)
	}
	...
	if err := migrations.Up0015(db); err != nil {
		t.Fatalf("repeated Up0015() error = %v", err)
	}
}
...
func TestProviderURLMigrationHandlesExistingColumn(t *testing.T) {
	...
	if err := migrations.Up0015(db); err != nil {
		t.Fatalf("Up0015() after pre-existing column error = %v", err)
	}
	if err := migrations.Validate0015(db); err != nil {
		t.Fatalf("Validate0015() after pre-existing column error = %v", err)
	}
}
...
func TestProviderURLMigrationRejectsMissingColumnInValidate(t *testing.T) {
	...
	if err := migrations.Validate0015(db); err == nil {
		t.Fatal("Validate0015() without column error = nil, want missing column error")
	} else if !strings.Contains(err.Error(), "provider_url") {
		t.Fatalf("Validate0015() error = %v, want column missing error", err)
	}
}
