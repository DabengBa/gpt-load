...
func TestCodexStandaloneSearchPreservesNativeHTTP(t *testing.T) {
	...
	transport := targetRoundTripper(func(request *http.Request) (*http.Response, error) {
		calls++
		if request.Method != http.MethodPost || request.URL.String() != "https://chatgpt.com/backend-api/codex/alpha/search" {
	...
	if err != nil || calls != 1 || string(response.Payload) != output || response.UpstreamRequestPath != "/backend-api/codex/alpha/search" {
