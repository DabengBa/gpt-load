	...
	tests := []struct {
	...
	}{
		{
			...
			assertPrompt: func(t *testing.T, body string) {
				...
				if !contains(body, `"max_tokens":3`) {
					t.Errorf("max_tokens=3 missing from body: %s", body)
				}
		...
		},
		{
			...
			assertPrompt: func(t *testing.T, body string) {
				...
				if !contains(body, `"max_tokens":3`) {
					t.Errorf("max_tokens=3 missing from multi-protocol gateway Chat: %s", body)
				}
...
func TestProbeFullFlowRejectsMalformedChatEvidence(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(writer http.ResponseWriter, _ *http.Request) {
		...
		_, _ = io.WriteString(writer, `{"id":"chat_1","choices":[{"message":{"role":"assistant","content":"4"}}]}`)
	}))
	defer server.Close()
	...
	if calls.Load() != 1 || result.Error == nil || result.Error.Kind != execution.ErrorKindProvider ||
		...
		t.Fatalf("calls/result = %d/%+v; want malformed Chat evidence rejected", calls.Load(), result)
	}
}
...
func TestProbeFullFlowRejectsMismatchedProtocolCarriers(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if calls.Load() != 1 || result.Error == nil || result.Error.Kind != execution.ErrorKindProvider ||
				result.DispatchState != execution.DispatchMaybeSent {
				t.Fatalf("calls/result = %d/%+v; want one dispatched request rejected by protocol-specific evidence", calls.Load(), result)
			}
		})
	}
}
...
func TestNonGenerativeProbeIsUnsupportedBeforeDispatch(t *testing.T) {
	...
	runtime := newProtocolTestRuntime(t, testRuntimeOptions{allowPrivateNetwork: true})
	spec := embeddingsProbeSpec(t, channel.OpenAICompatible, server.URL+"/v1")
	result := runtime.Execute(context.Background(), spec)
	if err := result.Validate(); err != nil {
		t.Fatalf("result validation: %v; result=%+v", err, result)
	}
...
func TestProbeTextEvidenceFromLiveAttempt(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name+"/empty_text_rejected", func(t *testing.T) {
			...
			if calls.Load() != 1 || result.Error == nil || result.StatusCode != http.StatusOK {
				t.Fatalf("calls/result = %d/%+v (expected provider error)", calls.Load(), result)
			}
			if result.Error.Kind != execution.ErrorKindProvider {
				t.Fatalf("error kind = %q, want provider", result.Error.Kind)
			}
		})
	}
}
...
func TestConvertedBedrockProbeUsesConverseWire(t *testing.T) {
	...
	runtime := newProtocolTestRuntime(t, testRuntimeOptions{allowPrivateNetwork: true})
	spec := utilitySpec(channel.AWSBedrock, protocol.Anthropic, execution.OperationProbe, "", "", nil)
	...
	if prepared.mode != channel.RouteConverted || prepared.responsesRequest == nil {
		t.Fatalf("Bedrock probe preparation = %#v; want converted Responses request", prepared)
	}

	conversionContext := schemas.NewBifrostContext(context.Background(), schemas.NoDeadline)
	defer conversionContext.Cancel()
	wire, err := bedrock.ToBedrockResponsesRequest(conversionContext, prepared.responsesRequest)
	if err != nil {
...
func TestNativeVertexProbeUsesGeminiWire(t *testing.T) {
	...
	if !ok || generationConfig["maxOutputTokens"] != float64(1) {
		t.Fatalf("Vertex generationConfig = %#v; want maxOutputTokens=1", payload["generationConfig"])
	}
