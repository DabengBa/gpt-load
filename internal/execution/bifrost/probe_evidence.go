...
// normalizeProbeAttemptResult turns a syntactically successful probe response
// into the protocol-level evidence R4 requires: the response must parse and
// contain non-empty generated text. An empty choices/output/candidates array or
// an unparseable body is reported as a provider failure, so a bare HTTP 2xx can
// never pass a probe.
func normalizeProbeAttemptResult(spec execution.AttemptSpec, result *execution.AttemptResult) {
	...
	if !spec.ClientProtocol.SupportsGeneratedText() {
		return
	}
	...
	*result = startedUnaryFailure(
		...
		"upstream returned no generated text for the probe",
	)
	result.UpstreamProtocol = upstreamProtocol
}
