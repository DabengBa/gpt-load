...
func TestCodexStandaloneSearchGateway(t *testing.T) {
	...
	if candidates := manager.Current().ExecutionCandidates[protocol.OpenAIResponses][execution.OperationWebSearch]["public"]; len(candidates) != 1 || candidates[0].GroupID != credential.GroupID {
	...
	if len(sink.events) != 1 || sink.events[0].Operation != execution.OperationWebSearch ||
