...
type websocketProvider interface {
	openWebsocket(execution.AttemptSpec, providerCredential, string) (execution.WebsocketSession, error)
}
...
func (a *Adapter) OpenWebsocket(ctx context.Context, spec execution.AttemptSpec) (execution.WebsocketSession, execution.WebsocketResult) {
	...
	// 环境代理无法解析成具体 URL，WS 合同要求显式代理或 direct，不做猜测式回退。
	if err != nil || settings.FromEnvironment {
		return reject()
	}
	if settings.URL == "" {
		settings.URL = "direct"
	...
	s, err := opener.openWebsocket(spec, credential, settings.URL)
	if err != nil {
...
func (*codexProviderBridge) openWebsocket(spec execution.AttemptSpec, credential providerCredential, proxyURL string) (execution.WebsocketSession, error) {
	...
	s, err := codex.NewWSSession(codex.WSSessionOptions{
		...
		ProxyURL: proxyURL, TurnTimeout: spec.Timeouts.Request,
...
func (s *codexWebsocketSession) ExecuteTurn(ctx context.Context, payload []byte, emit func(context.Context, []byte) error) execution.WebsocketResult {
	...
	if result.DispatchState == string(execution.DispatchMaybeSent) {
