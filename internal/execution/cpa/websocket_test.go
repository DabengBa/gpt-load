...
func TestWebsocketHTTPErrorEvidenceSurvivesCancellation(t *testing.T) {
	for _, status := range []int{401, 429} {
		...
		cancel()
		evidence := codexWebsocketEvidence(ctx, &codex.WSError{Code: "upstream_error", HTTPStatus: status, DispatchState: string(execution.DispatchNotSent)})
...
func TestWebsocketOnlyUnsent401RequestsRefresh(t *testing.T) {
	for _, dispatch := range []string{string(execution.DispatchNotSent), string(execution.DispatchMaybeSent)} {
		...
		if refresh != (dispatch == string(execution.DispatchNotSent)) {
