...
func TestCodexUpgradePreservesQuotaAndRejectionPolicy(t *testing.T) {
	for _, test := range []struct {
		...
		{
			name: "unary quota stays credential scoped", status: http.StatusTooManyRequests,
			...
			wantScope: execution.ErrorScopeCredential, wantReplay: true, wantEffect: health.EffectCooldownCredential,
		},
		{
			name: "stream quota stays credential scoped", stream: true, status: http.StatusTooManyRequests,
			errorJSON:  `{"type":"usage_limit_reached","resets_in_seconds":60}`,
			wantStatus: http.StatusTooManyRequests, wantHint: execution.FailureHintRateLimited,
			wantScope: execution.ErrorScopeCredential, wantReplay: true, wantEffect: health.EffectCooldownCredential,
		},
