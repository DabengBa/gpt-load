...
func TestFileRuntimeStateCheckpointRestoresAndConsumesFile(t *testing.T) {
	...
	if err := registry.ReplaceCredentials([]state.CredentialEntry{{
		ID: 1, GroupID: 10, Version: 1, IdentityGeneration: 1, Fingerprint: "test-1",
	...
	if err := loadedRegistry.ReplaceCredentials([]state.CredentialEntry{{
		...
		ID: 2, GroupID: 20, Version: 1, IdentityGeneration: 2, Fingerprint: "test-2",
...
func TestFileRuntimeStateCheckpointReturnsErrorWhenDeleteFails(t *testing.T) {
	...
	raw, err := json.Marshal(runtimeStateCheckpointDocument{
		Credentials: []state.CredentialRuntimeCheckpoint{{ID: 1, GroupID: 10, FailureCount: 37}},
	})
	if err != nil {
	...
	if got := registry.Snapshot()[0].FailureCount; got != 0 {
		t.Fatalf("failure count after failed checkpoint removal = %d, want 0", got)
	}
}
...
func TestFileRuntimeStateCheckpointConsumesMalformedFileAndReturnsError(t *testing.T) {
	...
	if got := registry.Snapshot()[0].FailureCount; got != 0 {
		t.Fatalf("failure count after malformed checkpoint = %d, want 0", got)
	}
	if _, err := os.Stat(path); !os.IsNotExist(err) {
