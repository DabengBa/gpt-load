...
- 最近 1 小时由单个小时桶改为 12 个 5 分钟桶。
- 补齐缺失空桶。
- 调整后端 usage 查询、前端趋势图、成本和三语文案。

**当前价值**

- 让监控页能观察短时流量、失败和成本变化。
- 对突发限流、异常费用和部署排障有直接帮助。

**合并评估**

- 难度：**L2**。
- 首个提交单独应用时冲突较少，但会与后续 `175949e2` 的时间窗口重构重叠。
- 应与 `175949e2` 作为一个 usage 子项目整体评估，避免先合并旧 API 再重复改写。

### `a97578fe` feat(access-keys): 完善访问密钥分发与用量管理

**内容**

- 访问密钥列表增加请求数、Token、成本和额度进度。
- 增加启停开关、复制、克隆、删除、额度重置、跳转用量和日志。
- 增加访问密钥 handoff 和分发相关 UI。

**当前价值**

- 访问密钥是多用户网关的主要运营入口。
- 按密钥查看用量和额度可以降低管理员排障与配额管理成本。
- 该能力与统一分组凭据契约基本正交。

**合并评估**

- 难度：**L2**。
- 需要适配当前访问密钥资源投影、Monitor URL 状态和 usage 查询。
- 不应覆盖当前 `MonitorView`、凭据管理和统一调度页面已有的 URL/缓存逻辑。
...
- 难度：**L2**。
- 通用 clipboard composable、fallback dialog 和基础组件可直接移植。
- `GroupCredentialRecord.vue` 与当前凭据管理页已被改造，需要保留当前组件逻辑后局部接入。
...
- 如果项目实际承接 Gemini 或 Antigravity 图片请求，这是完整的用户能力。
- 能复用现有 OpenAI Images 接口，减少客户端对渠道差异的感知。
- 如果当前部署不使用图片接口，优先级可低于用量、协议边界和访问密钥管理。

**合并评估**

- 难度：**L2**。
- 主要冲突集中在 `internal/execution/bifrost/executor.go`，但图片转换边界较独立。
- 需要先保留 `1898d8ee` 的转换失败分类，再重新接入当前执行器并运行图片全链路测试。
...
- 用精确 `from_ms`/`to_ms` 替换固定快捷范围作为查询核心。
- 支持非整点窗口和动态桶宽。
...
- 能把监控、日志和访问密钥用量串成同一时间上下文。

**合并评估**

- 难度：**L3**。
- 同时改变后端查询合同、前端资源投影、时间选择器、Monitor 路由和日志跳转。
- 与 `33fb54bf`、`a97578fe` 形成依赖链，应按最终 usage 设计整体重做，不要逐 commit 叠加。
...
1. 先移植 `2bdc1058` 和 `1898d8ee`，完成依赖与协议边界修复。
2. 将 `33fb54bf` 与 `175949e2` 合并评估为一个 usage 项目，保留当前 Monitor URL 和调度中心状态。
3. 选择性移植 `a97578fe` 与 `f0bd09b3`，优先保留访问密钥用量和复制回退能力。
4. 单独立项重做 `6da82242`、`96d3e0d5` 和 `d4699dd2`，先写当前领域合同和迁移方案。
5. 暂不直接合并 `e888fe60`、`9cb3f986`、`e0bfa07e`、`a4255546`；它们需要产品决策或架构重写。
6. 根据实际图片流量决定是否移植 `7ed01e66`。

## 当前判断

- 推荐直接接入：`2bdc1058`、`1898d8ee`。
- 推荐适配后接入：`33fb54bf`、`a97578fe`、`f0bd09b3`、`7ed01e66`。
- 推荐作为专题重做：`175949e2`、`6da82242`、`96d3e0d5`、`d4699dd2`。
- 当前不建议原样接入：`e888fe60`、`9cb3f986`、`e0bfa07e`、`a4255546`。

本 worktree 当前包含基于 `dev` 的 L1 选择性移植、测试合同适配和本分析文件，未执行整条 `origin/main` 合并。
