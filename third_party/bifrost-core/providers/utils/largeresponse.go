...
func FinalizeResponseWithLargeDetection(
	...
	//
	// The observer wraps only the read path (decompression input); idle-timeout
	// and cancellation teardown keep targeting the raw transport stream so
	// CloseWithError semantics are unchanged.
	bodyStream := httpObserverStateFromContext(ctx).wrapBody(rawBodyStream)
	...
	reader, stopIdleTimeout := NewIdleTimeoutReader(reader, rawBodyStream, GetStreamIdleTimeout(ctx), ctx)
	stopCancellation := SetupStreamCancellation(ctx, rawBodyStream, logger)
	cleanup := func() {
		stopCancellation()
		stopIdleTimeout()
		if wasGzip {
