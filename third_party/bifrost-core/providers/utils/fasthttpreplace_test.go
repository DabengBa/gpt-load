...
func TestFasthttpVersionIsConsistentAcrossModules(t *testing.T) {
	root, ok := tryRepoRoot()
	if !ok {
		// gpt-load vendors the core module alone (third_party/bifrost-core);
		// the upstream monorepo layout this guard walks does not exist there.
		t.Skip("bifrost monorepo root (core/framework/transports) not found; vendored core-only tree")
	}
	var checked int
...
// tryRepoRoot is the non-fatal variant of repoRoot, used so the vendored
// core-only tree can skip the monorepo-wide guard instead of failing.
func tryRepoRoot() (string, bool) {
	dir, err := os.Getwd()
	if err != nil {
		return "", false
	}
