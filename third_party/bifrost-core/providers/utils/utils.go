...
func ReleaseStreamingResponse(ctx *schemas.BifrostContext, resp *fasthttp.Response) {
	...
	if ctx.Err() != nil {
		// This close owner labels and completes the observation exactly as the
		// cancellation goroutine does: a parser that already returned leaves no
		// pending Read, so without this the attempt would carry no termination
		// evidence.
		termination := TerminationCancel
		if errors.Is(ctx.Err(), context.DeadlineExceeded) {
			termination = TerminationTimeout
	...
	if exhausted, _ := ctx.Value(schemas.BifrostContextKeyStreamBodyExhausted).(bool); !exhausted {
		...
		if parked || doesNotSendDoneMarker {
			state := httpObserverStateFromContext(ctx)
			state.markPendingTermination(TerminationClosed)
			closeBodyStream(bodyStream, errStreamParkedAfterFinish)
			state.completeIncomplete(errStreamParkedAfterFinish, TerminationClosed)
			return
		}
