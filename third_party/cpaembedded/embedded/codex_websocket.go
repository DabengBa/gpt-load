...
const (
	// 值域与 GPT-Load execution.DispatchState 的 not_sent / maybe_sent 一致。
	CodexWSNotSent   = "not_sent"
	CodexWSMaybeSent = "maybe_sent"

	codexWSSessionIDPrefix = "gptload-codex-ws-"

	defaultCodexWSTurnTimeout = 5 * time.Minute
	defaultCodexWSMaxBytes    = 10 << 20
)

// CodexWSSessionOptions 固定调用者已选择的身份和出站代理。
// ProxyURL 必须显式传入 direct 或代理 URL：CPA 的 WebSocket dialer 在代理为空时
// 会回落到 http.ProxyFromEnvironment，绕开调用方的代理来源决策。
...
func NewCodexWSSession(options CodexWSSessionOptions) (*CodexWSSession, error) {
	...
	auth := NewCodexAuth(options.CredentialID, options.Credential, "")
