export default {
  monitor: {
    title: '监控',
    description: '查看运行健康、请求日志和当前路由决策。',
    help: '查看说明',
    scrollHint: '可横向滚动查看全部列。',
    currentState: '当前状态',
    tabs: {
      label: '监控分区',
      health: '健康',
      usage: '用量与成本',
      inspector: '路由检查',
      schedule: '调度中心',
    },
    modelProbe: {
      title: '模型测活',
      description: '对每个目标发起一次低成本的真实上游问题请求，并将结果写入请求日志。',
      button: '测活',
      batch: '测活当前 {count} 行',
      draftHint: '先保存该模型行后再测活',
      loading: '正在测活…',
      requestFailed: '测活请求失败，已完成的结果仍然保留。',
      stopped: '已停止；已完成的结果保留。',
      progress: '已完成 {completed}/{total}',
      summary: '通过 {passed} · 未通过 {failed} · 无法判断 {inconclusive}',
      close: '关闭',
      stop: '停止',
      viewLog: '查看日志',
      credential: '本次使用凭据 {credential}',
      latency: '{value} 毫秒',
      notExecuted: '未执行上游请求，没有请求 ID',
      recovered: '已立即恢复',
      recoveryNotNeeded: '无需恢复状态',
      unknownValue: '—',
      disabledBadge: '已停用',
      disabledScope: {
        title: '包含已停用分组',
        description:
          '当前 {total} 行中有 {disabled} 个分组已停用。停用分组不参与调度，但仍可发起测活以确认上游可用性。',
        cancel: '取消',
        enabledOnly: '仅测启用分组（{count} 行）',
        includeDisabled: '一并测活（{count} 行）',
      },
      disabledConfirm: {
        title: '该分组已停用',
        description: '停用分组不参与调度。是否仍要发起一次测活？',
        cancel: '取消',
        confirm: '测活',
      },
      fields: {
        group: '分组',
        model: '模型',
        outcome: '结果',
        reason: '说明',
        protocol: '协议',
        routeMode: '路由模式',
        statusCode: '状态码',
        latency: '耗时',
        credential: '凭据',
        recovered: '恢复状态',
        logId: '请求 ID',
        testedAt: '测活时间',
      },
      outcome: {
        passed: '通过',
        failed: '未通过',
        inconclusive: '无法判断',
      },
      reason: {
        invalid_credential: '凭据无效',
        model_unavailable: '模型不可用',
        rate_limited: '被限流',
        timeout: '上游超时',
        upstream_error: '上游服务返回错误',
        probe_incompatible: '选定的协议或请求与该目标不兼容',
        no_answer: '上游未返回可用的答案文本',
        invalid_response: '上游响应不符合选定协议的格式',
        unknown: '原因未知',
        target_unavailable: '分组或模型不存在',
        no_schedulable_credential: '当前没有可调度的凭据',
      },
      routeMode: {
        native: '原生',
        converted: '协议转换',
      },
      toggle: {
        title: '依测活结果调整分组开关',
        hint: '勾选需要切换启用状态的分组，应用后将按结果启用或停用。',
        enablePassed: '通过的全部启用',
        disableNotPassed: '未通过 / 无法判断的全部停用',
        reset: '恢复原状',
        apply: '应用',
        applied: '已根据测活结果更新 {count} 个分组',
        applyFailed: '应用失败，请重试。',
        noChanges: '没有需要变更的分组。',
        groupEnabled: '已启用',
        groupDisabled: '已停用',
      },
    },
    health: {
      refresh: '刷新',
      loading: '正在加载运行健康…',
      loadFailed: '无法加载运行健康。',
      stale: '健康数据可能已过期；刷新失败，恢复倒计时已暂停。',
      none: '无',
      overview: {
        label: '健康状态概览',
        credentials: '凭据总数',
        credentialsDescription: '当前运行时有效凭据',
        available: '可用凭据',
        availableDescription: '可参与调度',
        cooldown: '冷却中',
        cooldownClear: '当前无需等待恢复',
        cooldownRecovery: '最早 {time} 后恢复',
        blacklisted: '已拉黑',
        blacklistedRecovery: '将在本地拉黑延迟结束后自动释放',
        blacklistedClear: '当前无拉黑凭据',
      },
      status: {
        empty: '尚无渠道凭据',
        unavailable: '当前无可用渠道凭据',
        exceptions: '存在运行时异常',
        clear: '当前无凭据运行时异常',
      },
      summary: {
        title: '运行健康',
        description: '当前凭据运行时快照公开的状态与原始计数。',
        revision: '修订 {revision}',
        observedAt: '观测时间 {time}',
      },
      counts: {
        total: '总数 {total}',
        available: '可用 {available}',
        cooldown: '冷却 {cooldown}',
        blacklisted: '拉黑 {blacklisted}',
        disabled: '停用 {disabled}',
      },
      groups: {
        title: '分组健康',
        description: '仅反映分组当前可用凭据，不对具体访问密钥、协议或模型作“可路由”承诺。',
        empty: '当前没有分组。',
        disabled: '已停用',
        emptyCredentials: '无凭据',
        unavailable: '不可用',
        attention: '需要关注',
        limited: '容量受限',
        available: '可用',
        tableLabel: '分组健康列表',
        columns: {
          group: '分组：{name}',
          status: '调度状态',
          credentialHealth: '凭据构成',
          exceptions: '当前异常',
          actions: '操作',
        },
        credentialHealthLabel:
          '共 {total} 个凭据：可用 {available}、冷却 {cooldown}、拉黑 {blacklisted}',
        cooldownCount: '{count} 冷却',
        blacklistedCount: '{count} 拉黑',
        none: '无',
        viewLogsFor: '查看 {name} 的请求日志',
        viewGroup: '查看分组 {name}',
        showAll: '查看全部 {count} 个分组',
        collapse: '收起分组',
      },
      problems: {
        title: '需要关注的凭据',
        description: '展示当前冷却或拉黑的凭据；近 {minutes} 分钟统计用于辅助判断。',
        count: '共 {count} 项',
        empty: '凭据状态正常',
        emptyDescription: '当前 {count} 个可用凭据均正常参与调度，暂无冷却或拉黑项。',
        emptyHint: '出现异常后会按影响调度的优先级显示在这里。',
        inactiveTitle: '当前没有凭据运行时异常',
        inactiveDescription: '暂无冷却或拉黑项；请结合顶部状态和分组健康检查可用性配置。',
        noneForStatus: '此状态下没有凭据。',
        cooldown: '冷却中',
        blacklisted: '已拉黑',
        remaining: '剩余 {time}',
        credentialMeta: '{group}',
        tableLabel: '需要关注的凭据列表',
        columns: {
          identity: '凭据 / 分组',
          status: '状态',
          window: '当前窗口',
          recovery: '恢复说明',
          actions: '操作',
        },
        window: '近 {minutes} 分钟：异常 {problems} · 成功 {successes}',
        windowCompactPrefix: '{minutes}min:',
        windowLabel: '统计窗口：近 {minutes} 分钟',
        consecutive: '连续异常 {count}',
        cooldownRecovery: '预计 {time} 后恢复',
        cooldownHint: '冷却结束后自动参与调度',
        scheduledReleaseRecovery: '预计 {time} 后释放',
        scheduledReleaseHint: '本地拉黑延迟结束后重新参与调度',
        viewLogs: '查看凭据 {credential} 的请求日志',
        viewGroup: '查看分组 {group}',
      },
      details: {
        failureCount: '累计失败',
        recentSuccessCount: '近期成功',
        recentFailureCount: '近期异常',
        consecutiveFailureCount: '连续异常',
        failureCategory: '失败原因',
        statusCode: 'HTTP 状态',
        statusUnavailable: '无 HTTP 状态',
        manualWeight: '手动权重',
        autoWeight: '自动权重',
        automatic: '自动',
      },
      recovery: {
        automatic: '自动恢复',
        notAutomatic: '非自动恢复',
        cooldownExpiry: '冷却到期',
        scheduledRelease: '定时释放拉黑状态',
        recoversAt: '自动恢复时间',
        seconds: '{seconds} 秒',
        minutesSeconds: '{minutes} 分 {seconds} 秒',
        hoursMinutes: '{hours} 小时 {minutes} 分',
        exact: '预计恢复时间：{time}',
      },
      failureCategories: {
        rate_limited: '触发限流',
        model_unavailable: '模型不可用',
        invalid_key: '密钥无效',
        upstream_host_error: '上游服务错误',
        client_error: '客户端请求错误',
        downstream_cancel: '下游已取消',
        authentication_required: '需要重新认证',
        ambiguous: '结果不明确',
      },
      accessKeyLimits: {
        title: '访问密钥费用限额',
        description:
          '当前因任一估算费用规则用尽而被阻断的访问密钥；这是正常业务状态，不代表服务异常。',
        total: '总额度',
        periodic: '每 {period}',
        periodDays: '{count} 天',
        periodHours: '{count} 小时',
        periodMinutes: '{count} 分钟',
        periodSeconds: '{count} 秒',
        temporary: '周期额度阻断',
        manual: '需要管理员处理',
        availableAgain: '可再次请求：',
        notAutomatic: '不会自动恢复',
        manage: '管理规则',
      },
      requestLog: {
        title: '请求日志采集',
        description: '当前进程生命周期计数，进程重启后归零。',
        normal: '采集正常',
        abnormal: '采集异常',
        retentionAbnormal: '清理异常',
        checkpointAbnormal: '额度快照异常',
        checkpointFailures: '快照写入失败',
        lastCheckpointFailureAt: '最近失败',
        checkpointRisk: '内存限制仍有效；重启后可能从较早快照恢复。',
        queue: '队列',
        queueDescription: '当前 / 容量',
        processTotal: '本进程累计',
        persistedDescription: '本进程已持久化',
        enqueued: '已入队',
        persisted: '已持久化',
        droppedNotRunning: '丢弃：未运行',
        droppedQueueFull: '丢弃：队列已满',
        droppedStopping: '丢弃：正在停止',
        droppedPersistFailed: '丢弃：持久化失败',
        droppedShutdown: '丢弃：关闭',
        droppedTotal: '丢弃总数',
        writeFailures: '写入失败',
        retentionFailures: '清理删除失败',
        queueDepth: '队列深度',
        queueCapacity: '队列容量',
        lastWriteFailureAt: '最近写入失败时间',
        lastRetentionFailureAt: '最近清理失败时间',
      },
    },
    usage: {
      loading: '正在加载用量报告…',
      loadFailed: '无法加载用量报告。',
      stale: '后台刷新失败，用量数据可能已过期。',
      options: {
        groupsFailed: '无法加载分组名称。可改为输入正整数分组 ID；用量报告不受影响。',
        partialFailed: '部分分组或渠道选项暂时无法加载；用量查询仍可正常使用。',
      },
      filters: {
        label: '用量报告筛选',
        title: '筛选用量与成本',
        description: '按分组、渠道、凭据与上游模型缩小当前统计范围。',
        close: '关闭用量与成本筛选',
        button: '筛选',
        range: '时间范围',
        range24h: '最近 24 小时',
        range30d: '最近 30 天',
        ranges: {
          '1h': '1 小时',
          '24h': '24 小时',
          '3d': '3 天',
          '7d': '7 天',
          '15d': '15 天',
          '30d': '30 天',
        },
        group: '分组',
        groupHelp: '可选，精确限定分组。',
        groupIdHelp: '分组名称加载失败；可输入正整数分组 ID。',
        anyGroup: '全部分组',
        channel: '渠道',
        anyChannel: '全部渠道',
        credential: '凭据：{name}',
        model: '上游模型',
        modelPlaceholder: '精确模型名',
        modelHelp: '可选，精确限定路由后的上游模型。',
        deletedOrUnknown: '#{id} · 已删除或未知',
        deletedOrUnknownGroup: '已删除或未知分组',
        refresh: '刷新',
        apply: '应用',
        reset: '重置',
        applied: '已应用筛选',
        anyModel: '全部上游模型',
        dirty: '筛选条件的更改尚未应用。',
        observedAt: '后端观测时间',
        refreshedAt: '最近刷新时间',
      },
      errors: {
        positiveId: '请输入有效的正整数分组 ID。',
        channelId: '请选择或输入有效的渠道 ID。',
        credentialId: '请输入有效的正整数凭据 ID。',
        model: '上游模型不能包含首尾空白或控制字符。',
      },
      scope: {
        title: '持久化用量与当前进程管道健康',
        description:
          '用量、Token 与预估成本来自所选 UTC 范围内的持久化请求日志。丢弃与写入失败是当前进程的累计计数，不跟随报告筛选条件。',
      },
      empty: {
        title: '没有匹配的用量数据',
        description: '请调整时间范围，或重置分组、渠道、凭据与上游模型筛选。',
      },
      kpi: {
        title: '用量摘要',
        description: '后端聚合观测时间：{time}。',
        requests: '请求数',
        outcomes: '成功与失败',
        outcomeCounts: '成功 {success} · 失败 {failure}',
        successRate: '成功率 {rate}',
        cacheHitRate: '缓存率',
        cacheTokenSummary: '缓存读取 {read} / 输入 {input}',
        totalTokens: '已报告 Token 总数',
        persistedWindow: '当前统计窗口',
        estimatedCost: '预估成本',
        estimatedCostBasis: '基于已报告 Token 与请求当时的倍率',
      },
      tokens: {
        cacheRead: '缓存读取',
      },
      quality: {
        title: '用量与持久化质量',
        description: '各项计数分开呈现，避免单一分数掩盖不完整用量。',
        windowDescription: '持久化质量计数只适用于所选时间窗内已持久化的记录。',
        missing: '用量缺失',
        partial: '用量部分缺失',
        unpriced: '成本未定价',
        pricingPartial: '部分定价',
        dropped: '请求丢弃',
        writeFailures: '写入失败',
        overlap: '用量缺失、部分缺失与未定价计数可能重叠，请勿将其相加作为请求总数。',
        partialExplanation: '用量部分上报时，已报告 Token 会计入 Token 总量；未报告部分保持未知。',
        unpricedExplanation: '未定价请求的已报告 Token 仍计入 Token 总量；成本只累计可定价部分。',
        aggregation:
          'Token 总量包含用量完整或部分上报的已知 Token；用量缺失与不适用不计入。成本仅累计可定价部分，可能存在未知成本。',
        openPrices: '查看模型价格',
      },
      process: {
        title: '当前进程采集健康',
        description: '这些当前进程计数不受报告筛选影响，并会在进程重启时重置。',
        lastWriteFailure: '最近写入失败',
        never: '尚无记录',
        clear: '当前进程未报告遥测丢弃或写入失败。',
        droppedWarning: '发生遥测丢弃时，统计只覆盖成功持久化的请求。',
        writeFailureWarning: '当前进程发生过 RequestLog 持久化批次失败。',
        warning: '当前进程采集存在异常：丢弃 {dropped}，写入失败 {failures}。',
      },
      trend: {
        tokensTitle: 'Token 与缓存趋势',
        tokensDescription: '总 Token 与缓存读取 Token 使用同一比例显示。',
        tokensAccessibleDescription: '后端返回的每个 UTC 桶中的总 Token 与缓存读取 Token。',
        costTitle: '预估成本趋势',
        costDescription: '按相同时间范围查看预估成本。',
        costAccessibleDescription: '后端返回的每个 UTC 桶中的预估成本。',
        metrics: {
          label: '趋势指标',
          tokens: 'Token',
          cost: '成本',
        },
        empty: '该范围内暂无请求时间桶。',
        hourly: '按小时',
        everyHours: '每 {count} 小时',
        daily: '按天',
        inputTokens: '输入 Token',
        outputTokens: '输出 Token',
      },
      series: {
        title: 'UTC 时间桶',
        description: '后端按所选小时或天粒度返回的聚合。',
        caption: '按 UTC 时间桶统计的用量聚合',
        disclosure: '查看时间桶明细',
      },
      distribution: {
        title: '消耗分布',
        description: '按模型、分组或访问密钥聚合展示主要消耗。',
        caption: '当前筛选范围内的消耗分布',
        limit: 'Top 5 + 其他',
        dimensionLabel: '分布维度',
        metricLabel: '排序指标',
        dimensions: { group: '分组', model: '模型', accessKey: '访问密钥' },
        metrics: { requests: '请求', tokens: 'Token', cost: '成本' },
        other: '其他',
        otherHint: '未进入 Top 5 的合计',
        otherGroupHint: '未进入 Top 5、已删除或未知分组的合计',
        otherAccessKeyHint: '未进入 Top 5、已删除或未知访问密钥的合计',
        groupValue: '分组 #{id}',
        accessKeyValue: '访问密钥 #{id}',
        deletedOrUnknownAccessKey: '#{id} · 已删除或未知',
        modelHint: '上游模型',
        unknownModel: '未知模型',
        requestsValue: '{value} 次',
        tokensValue: '{value} Token',
      },
      columns: {
        window: 'UTC 时间窗',
        requests: '请求数',
        success: '成功',
        failure: '失败',
        totalTokens: 'Token 总数',
        estimatedCost: '预估成本',
        quality: '质量',
        qualityCompact:
          '缺失 {missing} · 部分 {partial} · 未定价 {unpriced} · 部分定价 {pricingPartial}',
      },
      breakdown: {
        title: '模型与路由明细',
        description: '按模型及可用路由上下文展示当前筛选范围内的全部统计行。',
        caption: '按模型与路由统计的用量与成本明细',
        rowCount: '{count} 行',
        total: '总计',
        deletedOrUnknownChannel: '#{id} · 已删除或未知',
        columns: {
          model: '模型',
          group: '分组',
          channel: '渠道',
          attempts: '尝试数',
          attemptFailures: '尝试失败',
          successRate: '成功率',
          averageDuration: '平均耗时',
          averageFirstResponse: '平均首响',
          uncachedInput: '未缓存输入',
          cacheRead: '缓存读取',
          output: '输出',
        },
      },
    },
    inspector: {
      title: '路由检查',
      description: '按协议、模型与访问密钥检查当前候选分组和凭据。',
      boundary:
        '按所选协议模拟不含会话内容与亲和命中的标准无状态请求；Responses 使用 store:false，Images 只检查图片生成，Embeddings 只检查向量创建，Rerank 检查文本重排序。只读检查，不会发送上游请求或消耗 Token。',
      routeModes: {
        native: '原生',
        converted: '协议转换',
      },
      operations: {
        chat_completion: '对话补全',
        responses_create: '创建 Response',
        responses_retrieve: '读取 Response',
        responses_delete: '删除 Response',
        responses_cancel: '取消 Response',
        responses_input_items: '读取输入项',
        responses_compact: '压缩 Response',
        responses_input_tokens: '计算输入 Token',
        count_tokens: '计算 Token',
        responses_passthrough: 'Responses 扩展操作',
        images_generate: '生成图片',
        images_edit: '编辑图片',
        embeddings_create: '创建向量',
        rerank: '文档重排序',
      },
      routeRequirements: {
        any: '允许协议转换（可有损）',
        native: '必须使用原生路由',
      },
      form: {
        title: '模拟请求条件',
        label: '路由检查输入',
        protocol: '协议',
        selectProtocol: '请选择协议',
        operation: '操作',
        routeRequirement: '路由要求',
        model: '客户端模型',
        selectModel: '请选择模型',
        missingModelOption: '已不存在 · {model}',
        optional: '可选',
        modelPlaceholder: '输入客户端模型',
        modelOptional: '资源类协议请求可不填写。',
        accessKey: '访问密钥',
        selectAccessKey: '请选择访问密钥',
        accessKeyOption: '{name} · {status}',
        missingAccessKeyOption: '缺失的访问密钥',
        submit: '检查当前路由',
      },
      accessKeyStatus: {
        active: '启用',
        disabled: '停用',
      },
      options: {
        loading: '正在加载协议、模型与访问密钥选项…',
        failed: '无法加载路由检查选项。',
      },
      errors: {
        protocol: '请选择有效协议。',
        operation: '请选择与协议匹配的操作。',
        model: '请选择当前配置中存在的模型。',
        accessKey: '请重新选择现有访问密钥。',
        missingDeepLinkAccessKey: '链接中的访问密钥 #{id} 已不存在，请重新选择。',
        summary: '路由检查输入无效。',
      },
      request: {
        loading: '正在读取当前路由决策…',
        failed: '无法检查当前路由。',
      },
      empty: {
        title: '填写条件后检查当前路由',
        description: '检查结果会解释当前配置快照中的候选分组、凭据状态与排除原因。',
      },
      result: {
        title: '当前运行时观测',
        routableTitle: '当前请求可以路由',
        notRoutableTitle: '当前请求无法路由',
        routable: '可路由',
        notRoutable: '不可路由',
        reasonLine: '原因：{reason}',
        observedAt: '观测时间 {time}',
        routeStrategy: '路由策略：{strategy}',
        protocol: '协议',
        externalModel: '客户端模型',
        modelNotSpecified: '未指定',
        accessKey: '访问密钥',
        accessKeyStatus: '访问密钥状态',
        candidateGroups: '候选分组',
        availableCredentials: '当前可用凭据',
        reason: '原因',
        inputChanged: '输入已更改；下方仍显示上一次已提交输入的观测结果。',
        stale: '最近一次检查失败；下方是观测时间所示的较早结果。',
      },
      groups: {
        title: '候选分组',
        description: {
          native_first:
            '先尝试原生路由，原生无可用候选时使用协议转换；同层按有效权重加权随机。列表按层级和权重排序；请求亲和仍生效，权重份额不等于实际流量比例。',
          weighted_mix:
            '符合请求要求的原生与转换候选在同一池按有效权重加权随机，列表按权重排序。请求亲和仍生效，权重份额不等于实际流量比例。',
        },
        count: '{count} 个',
        tableLabel: '候选分组路由解释',
        completeEmpty: '未返回候选分组；这是该输入的完整当前运行时解释，并非空态。',
        included: '已纳入候选',
        excluded: '未纳入候选',
        standby: '后备层级',
        weightedCandidate: '当前参与加权',
        fallbackCandidate: '上层不可用时参与',
        priority: {
          native: 'P1 · 原生',
          converted: 'P2 · 协议转换',
        },
        standbyShare: '当前不参与竞争',
        availableTotal: '可用 / 总数',
        fallbackTier: '兜底层',
        entryCooldown: '条目冷却至',
        shareLabel: '{name} 的有效权重占比为 {share}',
        viewGroup: '查看分组',
        columns: {
          group: '分组',
          status: '路由模式',
          credentials: '凭据',
          weight: '条目权重',
          share: '权重份额',
          actions: '操作',
        },
      },
      credentials: {
        title: '凭据级解释',
        available: '可用',
        unavailable: '不可用',
        summary: '{available} 可用 · {unavailable} 不可用',
        tableLabel: '{name} 的凭据路由解释',
        noneReturned: '此分组未返回凭据候选。',
        cooldownUntil: '冷却到期时间',
        scheduledReleaseAt: '拉黑释放时间',
        none: '无',
        columns: {
          credential: '凭据',
          status: '状态',
          cooldown: '冷却到期',
        },
      },
      excluded: {
        title: '排除分组',
        description: '仅展示分组级排除结论；候选分组的凭据级原因可按需展开。',
        empty: '当前没有被排除的分组。',
        tableLabel: '排除分组路由解释',
        reasonCode: '原因代码',
      },
      reasons: {
        none: '无排除原因',
        access_key_disabled: '访问密钥已停用',
        access_key_expired: '访问密钥已过期',
        protocol_filtered: '访问密钥筛选排除了该协议',
        model_filtered: '访问密钥筛选排除了该模型',
        model_required_by_filter: '访问密钥模型筛选要求请求提供模型',
        operation_unsupported: '渠道不支持该协议操作',
        native_route_required: '该请求必须使用原生路由',
        no_route_target: '没有匹配该模型与协议的路由目标',
        group_disabled: '分组已停用',
        group_filtered: '访问密钥筛选排除了该分组',
        no_available_group: '当前没有可路由的候选分组',
        no_credentials: '分组没有运行时凭据',
        credential_blacklisted: '凭据已拉黑',
        credential_cooldown: '凭据正在冷却',
        credential_auth_unavailable: '订阅账号当前无法完成鉴权',
        credential_not_allowed: '该请求排除了此凭据',
        no_available_credential: '分组当前没有可用凭据',
        entry_blacklisted: '路由条目已拉黑',
        entry_cooldown: '路由条目正在冷却',
        entry_weight_zero: '路由条目权重为零',
        tier_demoted: '处于兜底优先级层待命',
        unknown: '未知路由原因',
      },
    },
    schedule: {
      panel: {
        model: '对外模型',
        mode: '模式',
        selectModel: '请选择模型',
        selectMode: '请选择模式',
        loadingOptions: '正在加载访问密钥…',
        optionsFailed: '无法加载访问密钥。',
        contextRequired: '请先选择完整调度上下文',
        kicker: '模型调度中心',
        contextReady: '上下文已就绪',
        context: '调度上下文',
        retry: '重试',
        indexFailed: '无法加载模型调度索引。',
        detailFailed: '无法加载模型调度详情。',
      },
      modes: {
        all: '全部候选',
        primary: '仅主层',
        fallback: '仅兜底层',
      },
      detail: {
        title: '调度详情',
        loading: '正在加载调度详情…',
        refresh: '刷新',
        stale: '显示较早的观测结果。',
        routeUnavailable: '当前路由不可用。',
        group: '分组',
        upstreamModel: '上游模型',
        weight: '权重',
        priority: '优先级',
        share: '当前占比',
        status: '状态',
        breakerRecovery: '熔断 / 恢复',
        cooldownUntil: '冷却到期时间',
        scheduledReleaseAt: '拉黑释放时间',
        clear: '清除',
        available: '可用',
        cooldown: '冷却中',
        blacklisted: '已拉黑',
        scheduledRelease: '拉黑释放',
        failures: '次失败',
        recover: '恢复',
        invalidValue: '请输入有效值。',
        derivedReadOnly: '派生条目只读',
        save: '保存',
        discard: '放弃更改',
        unsaved: '有未保存更改',
        saved: '已保存',
        saveFailed: '保存失败。',
        conflict: '配置已被其他管理员修改，请刷新后重试。',
        refreshToResolve: '刷新以解决',
        recoverFailed: '恢复失败。',
        noEntries: '当前没有条目。',
        unknownReason: '未知路由原因',
        draftPreview: '草稿预览',
        toggleEnabled: '切换分组启用状态',
        toggleFailed: '更新分组启用状态失败。',
        enabled: '已启用',
        disabled: '已停用',
        calls24h: '24 小时调用',
        successRate24h: '成功率',
        reasoning: '推理策略',
        groupDefault: '分组默认值',
        groupReasoningScope: '作用于本组全部 {count} 个条目中未设置条目覆盖的模型。',
        groupReasoningPreview: '{group}：保存后全组模型的有效推理策略',
        reasoningSaveBlocked: '不支持此策略，请修改后保存',
        entryOverride: '条目覆盖值',
        inherit: '未配置',
        effective: '有效值',
        source: '来源',
        capability: '支持状态',
        supported: '支持',
        unsupported: '不支持',
        capabilityUnknown: '未知',
        sourceEntry: '条目覆盖',
        sourceGroup: '分组默认',
        sourceClient: '客户端请求',
        sourceProviderDefault: 'Provider 默认',
        unsupportedEffortValue: '{value}（不支持）',
      },
      reasons: {
        access_key_disabled: '访问密钥已停用',
        access_key_expired: '访问密钥已过期',
        protocol_filtered: '访问密钥筛选排除了该协议',
        model_filtered: '访问密钥筛选排除了该模型',
        model_required_by_filter: '访问密钥模型筛选要求提供模型',
        operation_unsupported: '渠道不支持该协议操作',
        native_route_required: '该请求必须使用原生路由',
        no_route_target: '没有匹配的路由目标',
        group_disabled: '分组已停用',
        group_filtered: '访问密钥筛选排除了该分组',
        no_available_group: '当前没有可路由的候选分组',
        no_credentials: '分组没有运行时凭据',
        credential_blacklisted: '凭据已拉黑',
        credential_cooldown: '凭据正在冷却',
        credential_auth_unavailable: '订阅账号当前无法鉴权',
        credential_not_allowed: '该请求排除了此凭据',
        no_available_credential: '分组当前没有可用凭据',
        entry_blacklisted: '路由条目已拉黑',
        entry_cooldown: '路由条目正在冷却',
        entry_weight_zero: '路由条目权重为零',
        tier_demoted: '处于兜底优先级层待命',
      },
    },
    logs: {
      loading: '正在加载请求日志…',
      loadFailed: '无法加载请求日志。',
      loadMore: '加载更多',
      nextPageFailed: '加载下一页失败；已保留当前日志。',
      refresh: '刷新',
      refreshFailed: '刷新请求日志失败；已保留当前所有日志页。',
      stale: '最近一次刷新失败，当前显示最近一次成功结果。',
      caption: '请求日志',
      empty: {
        title: '尚无请求日志',
        description: '收到客户端请求后，脱敏日志会显示在这里。',
        filteredTitle: '没有匹配当前筛选条件的日志',
        filteredDescription: '调整或重置筛选条件后重试。',
      },
      options: {
        groupsFailed: '无法加载分组筛选选项；日志列表不受影响。',
        accessKeysFailed: '无法加载访问密钥筛选选项；日志列表不受影响。',
        partialFailed: '部分筛选选项暂时无法加载，日志查询仍可正常使用。',
      },
      filters: {
        label: '请求日志筛选',
        timeRange: '时间范围',
        from: '开始时间',
        to: '结束时间',
        quickRanges: '快捷时间范围',
        calendar: '日期范围',
        previousMonth: '上一个月',
        nextMonth: '下一个月',
        quickDisplay: {
          '1h': '最近 1 小时',
          '24h': '最近 24 小时',
          '3d': '最近 3 天',
          '7d': '最近 7 天',
          '15d': '最近 15 天',
          '30d': '最近 30 天',
        },
        quick: {
          today: '今天',
          yesterday: '昨天',
          '1h': '1h',
          '24h': '24h',
          '3d': '3d',
          '7d': '7d',
          '15d': '15d',
          '30d': '30d',
        },
        any: '全部',
        group: '任一尝试分组',
        groupHelp: '匹配任一尝试使用过的分组。',
        anyGroup: '全部尝试分组',
        searchGroups: '搜索分组',
        model: '客户端模型',
        clientModel: '客户端模型',
        searchClientModels: '搜索客户端模型',
        anyClientModel: '全部客户端模型',
        noMatches: '没有匹配项',
        upstreamModel: '任一尝试上游模型',
        accessKey: '访问密钥',
        affinityKey: '亲和作用域 Key',
        anyAccessKey: '全部访问密钥',
        protocol: '协议',
        anyProtocol: '全部协议',
        operation: '操作',
        anyOperation: '全部操作',
        stream: '流式请求',
        status: '状态',
        anyStatus: '全部状态',
        requestId: '请求 ID',
        finalStatusCode: '最终 HTTP 状态码',
        channel: '任一尝试渠道',
        anyChannel: '全部渠道',
        credential: '任一尝试凭据 ID',
        attemptStatusCode: '任一尝试状态码',
        failureCategory: '任一尝试失败分类',
        errorCode: '任一尝试错误码',
        retryStateLabel: '重试状态',
        modelConsistencyLabel: '返回模型',
        retryMin: '最少重试次数',
        retryMax: '最多重试次数',
        usageStateLabel: '用量状态',
        costStateLabel: '成本状态',
        completenessLabel: '计价完整性',
        cachePresent: '包含缓存 Token',
        more: '更多筛选',
        apply: '应用',
        reset: '重置',
        dirty: '筛选草稿尚未应用',
        applied: '已应用筛选',
        noneApplied: '无筛选条件',
        appliedFrom: '开始 {value}',
        appliedTo: '结束 {value}',
        appliedGroup: '尝试分组 {value}',
        appliedModel: '客户端模型 {value}',
        appliedClientModel: '客户端模型 {value}',
        appliedUpstreamModel: '尝试上游模型 {value}',
        appliedModelConsistency: '返回模型 {value}',
        appliedOperation: '操作 {value}',
        appliedAccessKey: '访问密钥 {value}',
        appliedAffinityKey: '亲和作用域 Key {value}',
        appliedChannel: '渠道 {value}',
        appliedCredential: '凭据 #{value}',
        appliedStatus: '状态 {value}',
        appliedUsageState: '用量 {value}',
        appliedCostState: '成本 {value}',
        appliedCompleteness: '计价 {value}',
        appliedRequestId: '请求 ID {value}',
        timezone: '本地时区（Local time）',
        lastRefreshed: '最近一次成功刷新',
        remove: '移除筛选条件 {value}',
        advancedTitle: '更多筛选',
        advancedDescription: '按请求、重试过程、最终结果和数值范围精确筛选。',
        closeAdvanced: '关闭更多筛选',
        sections: {
          request: '请求',
          attempt: '重试过程',
          result: '最终结果',
          ranges: '数值范围',
        },
        retryState: {
          retried: '发生过重试',
          not_retried: '未发生重试',
        },
        modelConsistency: {
          match: '一致',
          mismatch: '不一致',
          unknown: '未知',
        },
        usageState: {
          complete: '完整',
          partial: '部分缺失',
          missing: '缺失',
          not_applicable: '不适用',
        },
        costState: {
          priced: '已计价',
          unpriced: '未计价',
          not_applicable: '不适用',
        },
        completeness: {
          complete: '完整',
          partial: '部分计价',
          unavailable: '不可用',
          not_applicable: '不适用',
        },
        rangeFields: {
          retry_count_min: '最少重试次数',
          retry_count_max: '最多重试次数',
          first_response_min_ms: '最短首响（ms）',
          first_response_max_ms: '最长首响（ms）',
          duration_min_ms: '最短耗时（ms）',
          duration_max_ms: '最长耗时（ms）',
          input_tokens_min: '最少输入 Token',
          input_tokens_max: '最多输入 Token',
          output_tokens_min: '最少输出 Token',
          output_tokens_max: '最多输出 Token',
          cost_min_usd: '最低成本（USD）',
          cost_max_usd: '最高成本（USD）',
        },
      },
      errors: {
        dateTime: '请输入有效的本地日期时间。',
        range: '结束时间必须晚于开始时间。',
        positiveId: '请选择正的有效 ID。',
        channelId: '请选择或输入有效的渠道 ID。',
        model: '客户端模型不能包含首尾空白或控制字符。',
        status: '请选择有效状态。',
        requestId: '请输入规范的小写 UUIDv4。',
        nonNegativeInteger: '请输入非负整数。',
        numericRange: '最大值不能小于最小值。',
        text: '内容不能包含首尾空白或控制字符。',
        usd: '请输入最多 9 位小数的非负 USD 金额。',
        affinityKey: '请输入规范的 16 位前缀 + **** + 16 位后缀。',
      },
      deletedRef: '已删除 · #{id}',
      none: '无',
      yes: '是',
      no: '否',
      attemptCount: '{count} 次尝试',
      credentialRef: '凭据 #{id}',
      protocolConversion: {
        label: '协议转换：{client} → {upstream}',
        tooltip: '客户端协议：{client}\n上游协议：{upstream}',
        tooltipNotRecorded: '客户端协议：{client}\n上游协议：未记录',
        notRecorded: '未记录',
      },
      routeIdentity: {
        group: '分组：{name}',
        channel: '渠道：{name}',
        credential: '凭据：{name}',
        filterGroup: '只看分组 {name} 的日志',
        filterCredential: '只看凭据 {name} 的日志',
        openGroup: '查看分组 {name} 的维护页',
        openProviderUrl: '在新标签页打开供应商官网 {url}',
      },
      filterAccessKey: '只看访问密钥 {name} 的日志',
      filterAffinityKey: '只看亲和作用域 Key {value} 的日志',
      copyAffinityKey: '复制完整脱敏亲和作用域 Key',
      copySuccess: '已复制',
      copyFailure: '复制失败',
      resultSummary: '本页 {count} 条 · 最新优先',
      filterModel: '只看模型 {name} 的日志',
      details: '查看详情',
      failureCategory: {
        ok: '正常',
        rate_limited: '触发限流',
        model_unavailable: '模型不可用',
        invalid_key: '密钥无效',
        upstream_host_error: '上游主机错误',
        client_error: '客户端错误',
        conversion_unsupported: '协议转换不支持',
        downstream_cancel: '下游取消',
        authentication_required: '需要重新认证',
        ambiguous: '原因不明确',
        unknown: '未知分类',
      },
      action: {
        terminate: '终止',
        retry: '重试',
        cooldown_credential: '冷却凭据',
        fail_credential: '记录凭据失败',
        skip_group: '跳过分组',
        unknown: '未知动作',
      },
      failureOrigin: {
        client: '客户端',
        upstream: '上游',
        downstream: '下游连接',
        internal: '网关内部',
      },
      failureScope: {
        request: '当前请求',
        model: '模型',
        credential: '凭据',
        group: '分组',
      },
      retryDirective: {
        none: '不重试',
        refresh_credential: '刷新当前凭据后重试',
        next_candidate: '尝试下一个候选',
      },
      effect: {
        none: '无运行态变更',
        cooldown_credential: '冷却凭据',
        record_credential_failure: '记录凭据失败',
        skip_group: '跳过分组',
      },
      operation: {
        chat_completion: '聊天补全',
        responses_create: '创建 Response',
        responses_retrieve: '获取 Response',
        responses_delete: '删除 Response',
        responses_cancel: '取消 Response',
        responses_input_items: '获取输入项',
        responses_compact: '压缩 Response',
        responses_input_tokens: '计算输入 Tokens',
        count_tokens: '计算 Tokens',
        responses_passthrough: 'Responses 资源透传',
        images_generate: '生成图片',
        images_edit: '编辑图片',
        embeddings_create: '创建向量',
        rerank: '文档重排序',
        list_models: '列出模型',
        probe: '健康探测',
        web_search: 'Codex 独立搜索',
      },
      dispatchState: {
        not_sent: '未发送到上游',
        maybe_sent: '可能已发送，尚未开始响应',
        response_started: '已收到上游响应',
      },
      usage: {
        missing: '用量缺失',
        not_applicable: '不适用',
      },
      cost: {
        unpriced: '成本未知',
        not_applicable: '不适用',
      },
      drawer: {
        title: '请求日志详情',
        description: '查看最终结果和脱敏的上游尝试链。',
        descriptionSelfScoped: '查看当前访问密钥的请求、结果与用量。',
        close: '关闭请求日志详情',
        loading: '正在加载请求日志详情…',
        loadFailed: '无法加载请求日志详情。',
        summary: '请求结果',
        requestId: '请求 ID',
        copyRequestId: '复制请求 ID',
        copyCredential: '复制凭据标识',
        completedAt: '完成时间',
        status: '状态',
        protocol: '协议',
        accessKey: '访问密钥',
        clientModel: '客户端模型',
        upstreamModel: '上游模型',
        modelNotSpecified: '未指定',
        reasoningConfig: '思考配置',
        reasoningMode: '思考模式',
        reasoningEffort: '思考级别',
        reasoningBudget: '思考预算',
        reasoningBudgetValue: '{value} Tokens',
        reasoningBudgetDynamic: 'auto',
        reasoningBudgetDisabled: 'off',
        reasoningNotSpecified: '未指定',
        appliedReasoning: '转换后思考参数',
        statusCode: '状态码',
        duration: '耗时',
        firstResponse: '首响',
        outputRate: '输出速率',
        attemptCount: '尝试次数',
        request: '客户端请求',
        finalExecution: '上游执行',
        route: '请求与最终路由',
        routeIdentity: '路由标识',
        upstreamProtocol: '上游协议',
        affinity: '亲和命中',
        errorCode: '错误码',
        errorSummary: '错误消息',
        keyReason: '关键原因',
        keyReasonFromAttempt: '关键原因 · 尝试 #{sequence}',
        requestErrorCode: '最终错误码',
        expandErrorMessage: '展开完整错误消息',
        collapseErrorMessage: '收起错误消息',
        openInspector: '按当前状态检查（非历史重放）',
        attempts: '尝试链',
        noAttempts: '此请求没有上游尝试。',
        attempt: '尝试 #{sequence}',
        attemptDetails: '详细诊断',
        clientStreamState: '客户端流状态',
        clientStreamStarted: '已开始向客户端输出',
        clientStreamNotStarted: '尚未向客户端输出',
        group: '历史分组',
        channel: '历史渠道',
        credential: '历史凭据',
        operation: '操作类型',
        routeMode: '路由模式',
        dispatchState: '派发状态',
        upstreamRequestId: '上游请求 ID',
        failureCategory: '失败分类',
        failureOrigin: '错误来源',
        failureScope: '影响范围',
        retryDirective: '重试决策',
        effect: '运行态效果',
        ruleId: '命中规则',
        action: '动作',
        gatewayAction: '网关动作',
        subsequentAttempt: '后续尝试',
        subsequentAttemptOccurred: '已发生',
        noSubsequentAttempt: '无',
        modelObservation: '上游返回模型观测',
        requestedModel: '上游请求模型',
        reportedModel: '上游返回模型',
        usage: {
          title: '用量成本',
          description: '最终持久化的用量字段。成本始终为预估值，未知不代表免费。',
          finalGroup: '最终分组',
          groupId: '分组 #{id}',
          usageStateLabel: '用量状态',
          costStateLabel: '成本状态',
          estimatedCost: '预估成本',
          unknown: '未知',
          notApplicable: '不适用',
          openPrices: '查看该上游模型的价格',
          state: {
            complete: '用量完整',
            partial: '用量部分缺失',
            missing: '用量缺失',
            not_applicable: '用量不适用',
          },
          costState: {
            priced: '可计算预估成本',
            unpriced: '预估成本未知',
            not_applicable: '成本不适用',
          },
          aggregation: {
            completePriced: '计入默认 Token 与预估成本聚合。',
            completeUnpriced: '已报告 Token 计入 Token 聚合；成本未知。',
            partialPriced: '已报告 Token 与已知成本分别计入对应聚合；其余部分未知。',
            partialUnpriced: '已报告 Token 计入 Token 聚合；成本未知。',
            missing: '用量缺失，Token 与成本均不计入聚合。',
            notApplicable: '因用量与成本均不适用，不计入默认 Token 与预估成本聚合。',
          },
          tokens: {
            uncachedInput: '未缓存输入',
            cacheRead: '缓存读取',
            cacheWrite5m: '缓存写入（5 分钟）',
            cacheWrite1h: '缓存写入（1 小时）',
            output: '输出',
          },
        },
      },
      columns: {
        time: '时间',
        timeNewestFirst: '时间，最新优先',
        modelProtocol: '模型/协议',
        response: '响应',
        cost: '成本',
        tokens: 'Tokens',
        tokensDetail: 'Tokens（输入 / 输出；缓存命中率可查看说明）',
        completedAt: '完成时间',
        requestId: '请求 ID',
        route: '路由',
        affinityKey: '亲和作用域 Key',
        accessKey: '访问密钥',
        result: '最终状态',
        timing: '首响/耗时',
        timingDetail: '首响时间 / 总耗时；悬停查看输出速率',
        actions: '操作',
      },
      status: {
        success: '成功',
        error: '错误',
        incomplete: '未完成',
        canceled: '已取消',
      },
      response: {
        normal: '正常',
        streamError: '流错误',
        errorWithCode: '错误 {code}',
        httpStatus: 'HTTP {code}',
      },
      responseTooltip: {
        status: '最终状态：{status}',
        attempts: '尝试次数：{count}',
        reason: '关键原因：{reason}',
        errorCode: '错误码：{code}',
      },
      modelConsistency: {
        notObserved: '未观察到',
        unknownLabel: '未观察到上游返回模型',
        mismatchLabel: '模型不一致',
        unknownTooltip:
          '上游请求模型：{upstream}\n上游返回模型：未观察到\n提示：未观察到上游返回模型',
        mismatchTooltip: '上游请求模型：{upstream}\n上游返回模型：{reported}\n告警：模型不一致',
      },
      affinitySourceLabel: '亲和来源：{source}',
      affinityStateLabel: '亲和状态：{state}',
      continuityHit: '续接：已采用',
      affinitySource: {
        none: '无',
        prompt_cache_key: 'prompt_cache_key',
        prompt_prefix: 'prompt_prefix',
      },
      affinityState: {
        no_signal: '未评估或无信号',
        cache_miss: '未找到亲和绑定',
        hit: '已解析亲和绑定（含持久恢复）',
        group_disabled: '分组已禁用亲和',
        target_unavailable: '目标凭据不可用',
        cache_unavailable: '亲和解析不可用（缓存、配置或存储）',
      },
      reasoning: {
        compact: '[{value}]',
        disabled: '[off]',
        dynamic: 'auto',
      },
      tokens: {
        input: '输入',
        output: '输出',
        cacheRead: '缓存读取',
        cacheWrite5m: '缓存写入 5m',
        cacheWrite1h: '缓存写入 1h',
        cacheWrite: '缓存写入',
        cacheHitRate: '缓存率',
        cacheDetails: '查看缓存 Token 明细',
        partial: '当前 Token 用量不完整',
      },
      receipt: {
        total: '已计价合计',
        baseTotal: '原始已计价合计',
        finalTotal: '最终已计价合计',
        totalRounding:
          '各计价项按原规则四舍五入后合计，再同时乘以分组和访问密钥倍率，最后四舍五入到 0.000000001 USD；未定价部分不计入合计。',
        rounding:
          '各计价项应用全部倍率后四舍五入到 0.000000001 USD，再相加；未定价部分不计入合计。',
        formula: '成本计算',
        input: '输入',
        output: '输出',
        identity: '计价身份',
        historicalSchema1: '历史作用域',
        historicalSchema2: '历史全局价格',
      },
      pricingMode: {
        fastLabel: 'Fast 模式计价',
        tierLabel: '命中档位：输入 Token ≥ {threshold}',
      },
    },
  },
} as const
