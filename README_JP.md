...
- **可観測で導入しやすく、データを自己管理** — 健全性、ルート、ログ、使用量、コスト概算とログ一覧でのキャッシュヒット率を確認でき、SQLite、PostgreSQL とローカル認証情報暗号化を単一バイナリで利用できます。
...
統一された `DATABASE_DSN` で SQLite または PostgreSQL に接続します：
...
| `DEBUG_CAPTURE_ENABLED` | `false` | オプションの平文アプリケーション層通信捕捉。Gateway/CPA が観測したリクエストとレスポンスの Header、Body、結果を 12 時間保持して自動削除します。管理者のみアクセス可能です。信頼できない共有環境では有効にしないでください。 |
...
- デバッグ通信捕捉は既定で無効です。`DEBUG_CAPTURE_ENABLED=true` にすると、独立した捕捉ストレージに観測された平文のリクエストとレスポンスの Header、Body が固定 12 時間保存され、資格情報や Cookie を含む場合があります。詳細と ZIP ダウンロードは管理者向け API のみで提供されます。`AUTH_KEY`、データベース、バックアップ、エクスポートした ZIP を保護してください。捕捉範囲は Gateway と CPA 統合が実際に観測したアプリケーション層データです。Bifrost の低レベル HTTP transport、TLS/socket wire、または観測されなかったデータの捕捉を保証しません。
...
- Responses `create` はオプションのトップレベルフィールド `prompt_cache_key` をソフトアフィニティシグナルとして受け入れます。値は空でなく、有効な UTF-8 で、先頭/末尾に空白や制御文字がなく、256 バイト以下である必要があります。無効な値はリクエストをブロックせずに静かに無視されます。有効な場合、明示的なキーは `instructions`/`system`/`developer` と最初の `user`/`input_text` から派生されるプロンプトプレフィックスシグナルより優先されます。生のキーや派生値はログに記録されたり公開されたりしません。ソフトアフィニティは適格な候補の順序を変更するだけで、資格情報セットを拡大したり、`previous_response_id` のハード継続を上書きしたりしません。リクエストログには、制限付きの `affinity_source`、`affinity_state`、および独立した `continuity_hit` インジケーターが表示されます。これはプロセスローカルの最適化であり、厳密なセッション ID、マシンバインディング、またはインスタンス間共有を意味するものではありません。
